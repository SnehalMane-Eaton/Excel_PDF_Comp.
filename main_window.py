...
class MainWindow:
    ...
    def _build_folder_section(
        ...
        # BOM1 / BOM2 ORDER
        bom_order_frame = ttk.LabelFrame(
            section,
            text="BOM1 / BOM2 ORDER",
            padding=6,
        )
        bom_order_frame.grid(
            row=2,
            column=0,
            columnspan=3,
            sticky="ew",
        ...
        ttk.Label(
            bom_order_frame,
            text="BOM1:",
        ).grid(row=0, column=0, padx=(5, 4), pady=4, sticky="w")
        ...
        self.bom1_combo.grid(
            row=0, column=1, padx=(0, 15), pady=4
        )
        self.bom1_combo.bind(
            "<<ComboboxSelected>>",
            self._on_bom1_changed,
        )

        ttk.Label(
            bom_order_frame,
            text="BOM2:",
        ).grid(row=0, column=2, padx=(5, 4), pady=4, sticky="w")
        ...
        self.bom2_combo.grid(
            row=0, column=3, padx=(0, 5), pady=4
        )
    ...
    def _on_bom1_changed(self, _event=None):
        """Keep BOM2 opposite to BOM1."""
        if self.bom1_type.get() == "PDF":
            self.bom2_type.set("Excel")
        else:
            self.bom1_type.set("Excel")
            self.bom2_type.set("PDF")
